...
export default function ApprovalActionButtons({ doc, lines }: { doc: any, lines: any[] }) {
  ...
  const isAdmin = String(currentUser?.role_name || '').toUpperCase() === 'ADMIN' || currentUser?.role === 'admin';
  ...
  const handleApproveCancellation = async () => {
    ...
    try {
      ...
      if (activeLine.line_no === 1) {
        ...
        await supabase.from('approval_lines').update({ status: 'waiting', acted_at: null, opinion: null }).eq('id', activeLine.id);
  ...
  const handleFinalizeCancel = async () => {
    ...
    try {
      ...
      // 2. 문서 상태를 'rejected'로 업데이트하여 종결 (SQL 내부에서 이미 바꿨더라도 확실히 동기화)
      await updateDoc({ 
        status: 'rejected', 
        remarks: '취소 완료(재고환원)',
      ...
      router.push('/approvals'); // 결재함으로 이동
    } catch (e: any) { 
      console.error(e);
        ...
        <button 
          onClick={async () => {
            ...
            try {
              ...
              window.location.reload();
            } catch (e: any) { alert('관리자 취소 오류: ' + e.message); }
          }} 
