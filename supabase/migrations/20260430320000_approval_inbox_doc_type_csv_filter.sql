-- approval_inbox_query: p_doc_type 에 콤마 구분 다중 doc_type 허용
-- 예) p_doc_type = 'draft_doc,purchase_request,leave_request'
BEGIN;
  ...
  ranked AS (
    SELECT
      ...
      row_number() OVER (
        ORDER BY
          CASE
            WHEN lower(btrim(coalesce(f.status::text, ''))) = 'approved' THEN 1
            ELSE 0
          END ASC,
      ...
      (
        SELECT jsonb_agg(
          ...
          ORDER BY
            (
              CASE
                WHEN lower(btrim(coalesce(r.status::text, ''))) = 'approved' THEN 1
                ELSE 0
              END
            ) ASC,
