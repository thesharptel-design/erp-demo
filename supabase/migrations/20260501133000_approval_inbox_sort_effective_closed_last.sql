-- approval_inbox_query: treat approved/effective/closed as completed documents for inbox sorting.
BEGIN;
...
AS $$
  ...
  base AS (
    ...
    WHERE auth.uid() IS NOT NULL
      AND public.can_read_approval_doc(d.id, auth.uid())
  ),
  filtered AS (
    SELECT *
    FROM base b
    WHERE
      (
        ...
        OR b.doc_type IS NOT DISTINCT FROM NULLIF(btrim(p_doc_type), '')
        OR (
          strpos(coalesce(p_doc_type, ''), ',') > 0
          AND b.doc_type = ANY (
            ARRAY(
              SELECT NULLIF(btrim(x), '')
              FROM unnest(string_to_array(coalesce(p_doc_type, ''), ',')) AS x
      ...
      AND (
        ...
        OR strpos(
          lower(
            coalesce(nullif(trim(b.writer_user_name::text), ''), '—')
  ...
  ranked AS (
    SELECT
      ...
      row_number() OVER (
        ORDER BY
          CASE
            WHEN lower(btrim(coalesce(f.status::text, ''))) IN ('approved', 'effective', 'closed') THEN 1
            ELSE 0
          END ASC,
          f.drafted_at DESC NULLS LAST,
          f.id DESC
      ) AS rn
    FROM filtered f
  ),
      ...
      (
        SELECT jsonb_agg(
          ...
          ORDER BY
            (
              CASE
                WHEN lower(btrim(coalesce(r.status::text, ''))) IN ('approved', 'effective', 'closed') THEN 1
                ELSE 0
              END
            ) ASC,
            r.drafted_at DESC NULLS LAST,
            r.id DESC
        )
        FROM rows_ordered r
      ),
