...
ALTER TABLE public.inbound_tracking_completions
  ADD COLUMN IF NOT EXISTS completed_qty numeric NOT NULL DEFAULT 1;
...
REVOKE ALL ON FUNCTION public.execute_inbound_tracking_completion(bigint, bigint, numeric, uuid, text, text, date, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.execute_inbound_tracking_completion(bigint, bigint, numeric, uuid, text, text, date, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.execute_inbound_tracking_completion(bigint, bigint, numeric, uuid, text, text, date, text) TO service_role;
