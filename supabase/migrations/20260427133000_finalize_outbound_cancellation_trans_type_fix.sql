...
-- `inventory_transactions.trans_type` CHECK와 정합:
-- 취소 환원 행은 커스텀 값(CANCEL_IN) 대신 표준 IN으로 기록한다.
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.inventory_transactions (
      ...
      r_tx.item_id, 'IN', r_tx.qty, r_tx.lot_no, r_tx.exp_date, r_tx.serial_no,
...
COMMENT ON FUNCTION public.finalize_outbound_cancellation(bigint) IS
  'Restores stock for outbound cancellation and writes reversal as IN transaction.';
