-- 통합결재문서함 정렬: 모든 doc_type에 동일 적용
-- (최종승인은 목록 하단, 그 외는 id 내림차순 = 최신이 위). 출고요청만 예외 두지 않음.
BEGIN;
...
AS $$
  ...
  ranked AS (
    SELECT
      ...
      row_number() OVER (
        ORDER BY
          CASE WHEN f.status::text = 'approved' THEN 1 ELSE 0 END ASC,
          f.id DESC
      ) AS rn
    FROM filtered f
  ),
    ...
    'items', coalesce(
      (
        SELECT jsonb_agg(
          to_jsonb(r)
          ORDER BY
            (CASE WHEN r.status::text = 'approved' THEN 1 ELSE 0 END) ASC,
            r.id DESC
        )
        FROM rows_ordered r
      ),
