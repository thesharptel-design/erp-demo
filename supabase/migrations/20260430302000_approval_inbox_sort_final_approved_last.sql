-- 통합결재문서함 `approval_inbox_query`:
-- 1) 최종승인(approval_docs.status = approved)은 항상 목록 하단 그룹.
-- 2) 그 외 문서는 기안일 최신순, 동일일자는 id 내림차순.
-- 3) status 비교는 lower(trim)로 보강 (enum/공백 이슈 방지).
-- 이전에 `ORDER BY id DESC`만 있던 DB에도 이 마이그레이션 한 번이면 정렬이 맞습니다.
BEGIN;
